...
pub fn unescape_and_validate_uuid_internal(ark_url: &str, escaped_uuid: &str) -> PyResult<String> {
    ...
    if !is_valid(&unescaped_uuid)? {
        return Err(PyValueError::new_err(format!("Invalid ARK ID: {}", ark_url)));
    }
    
    // Return UUID without the check digit (remove last character)
    if unescaped_uuid.is_empty() {
        return Err(PyValueError::new_err(format!("Empty UUID in ARK ID: {}", ark_url)));
    }
...
mod tests {
    ...
    fn test_known_python_parity_cases() {
        ...
        let test_cases = vec![
            ("0001-12345678-abcd-ef12-3456-789012345678", "0001=12345678=abcd=ef12=3456=789012345678U"),
            ("0002-70aWaB2kWsuiN6ujYgM0ZQ", "0002=70aWaB2kWsuiN6ujYgM0ZQ5"),
            ("0003-abc123def456", "0003=abc123def4560"),
            ("000112345678abcdef123456789012345678", "000112345678abcdef1234567890123456789"),
            ("a-b-c-d-e", "a=b=c=d=e8"),
            ("test-uuid-with-many-hyphens-here", "test=uuid=with=many=hyphens=here7"),
            ("singleword", "singlewordn"),
            ("0000-0000-0000-0000-0000-000000000000", "0000=0000=0000=0000=0000=000000000000A"),
        ];
        
        for (input, expected) in test_cases {
            let result = add_check_digit_and_escape_internal(input).unwrap();
            assert_eq!(result, expected, "Python parity failed for input: {}", input);
        }
    }
    ...
    fn test_cross_implementation_compatibility() {
        ...
        for (original_uuid, python_escaped) in python_generated_cases {
            ...
            assert_eq!(rust_escaped, python_escaped, "Rust should match Python output for: {}", original_uuid);
            
            // Our Rust implementation should be able to process Python-generated escaped UUIDs
            let recovered = unescape_and_validate_uuid_internal("ark:/test", &python_escaped).unwrap();
            assert_eq!(recovered, original_uuid, "Rust should recover Python-escaped UUID: {}", python_escaped);
        }
    }
}