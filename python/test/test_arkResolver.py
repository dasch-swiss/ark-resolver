...
def settings():
    ...
    try:
        with open(os.path.join("python", "test", "settings.pkl"), "rb") as mock_settings:
            return pickle.load(mock_settings)
    except FileNotFoundError:
        # When running tests directly from the IDE, the working directory might be different
        with open("settings.pkl", "rb") as mock_settings:
            return pickle.load(mock_settings)
...
def test_ark_url_info_redirect_resource(settings):
    ...
    ark_url_info = ArkUrlInfo(settings,
...
def test_ark_url_info_redirect_salsah_project(settings):
    ...
    ark_url_info = ArkUrlInfo(settings, "https://ark.example.org/ark:/00000/1/0803")
    redirect_url = ark_url_info.to_redirect_url()
    ...
    ark_url_info = ArkUrlInfo(settings, "https://ark.example.org/ark:/00000/1/0006")
    redirect_url = ark_url_info.to_redirect_url()
...
def test_ark_url_info_redirect_salsah_ark(settings):
    ...
    ark_url_info = ArkUrlInfo(settings, "http://ark.example.org/ark:/00000/0002-779b9990a0c3f-6e")
    redirect_url = ark_url_info.to_redirect_url()
    ...
    ark_url_info = ArkUrlInfo(settings, "http://ark.example.org/ark:/00000/0002-779b9990a0c3f-6e.20190129")
    redirect_url = ark_url_info.to_redirect_url()
