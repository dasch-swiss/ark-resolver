...
def test_ark_url_formatter(settings):
    ...
    assert (
        ark_url == "https://ark.example.org/ark:/00000/1/0001/cmfk1DMHRBiR4=_6HXpEFAn"
    )

    # generate an ARK URL from a resource IRI with a timestamp
    ark_url = ark_url_formatter.resource_iri_to_ark_url(
        resource_iri=resource_iri, timestamp="20180604T085622513Z"
    )
    assert (
        ark_url
    ...
    ark_url = ark_url_formatter.resource_iri_to_ark_url(
        resource_iri=resource_iri, value_id=value_id
    )
    assert (
        ark_url
    ...
    ark_url = ark_url_formatter.resource_iri_to_ark_url(
        resource_iri=resource_iri, value_id=value_id, timestamp="20180604T085622513Z"
    )
    assert (
        ark_url
        == "https://ark.example.org/ark:/00000/1/0001/cmfk1DMHRBiR4=_6HXpEFAn/pLlW4ODASumZfZFbJdpw1gu.20180604T085622513Z"
    )
...
def test_ark_url_info_redirect_resource(settings):
    ...
    ark_url_info = ark_url_rust.ArkUrlInfo(
        settings, "ark:/00000/1/0001/cmfk1DMHRBiR4=_6HXpEFAn"
    )
    redirect_url = ark_url_info.to_redirect_url()
    ...
    ark_url_info = ark_url_rust.ArkUrlInfo(
        settings, "ark:/00000/1/0001/cmfk1DMHRBiR4=_6HXpEFAn.20180604T085622513Z"
    )
    redirect_url = ark_url_info.to_redirect_url()
    assert (
        redirect_url
        == "http://0.0.0.0:4200/resource/0001/cmfk1DMHRBiR4-_6HXpEFA?version=20180604T085622513Z"
    )

    # parse and redirect an ARK URL of a DSP resource with a timestamp without a fractional part
    ark_url_info = ark_url_rust.ArkUrlInfo(
        settings, "ark:/00000/1/0001/cmfk1DMHRBiR4=_6HXpEFAn.20180604T085622Z"
    )
    redirect_url = ark_url_info.to_redirect_url()
    assert (
        redirect_url
        == "http://0.0.0.0:4200/resource/0001/cmfk1DMHRBiR4-_6HXpEFA?version=20180604T085622Z"
    )

    # parse an ARK URL of a DSP resource without a timestamp and redirect it to a customized location
    ark_url_info = ark_url_rust.ArkUrlInfo(
        settings, "ark:/00000/1/0005/0_sWRg5jT3S0PLxakX9ffg1"
    )
    redirect_url = ark_url_info.to_redirect_url()
...
def test_ark_url_info_redirect_value(settings):
    ...
    ark_url_info = ark_url_rust.ArkUrlInfo(
        settings, "ark:/00000/1/0005/SQkTPdHdTzq_gqbwj6QR=AR/=SSbnPK3Q7WWxzBT1UPpRgo"
    )
    redirect_url = ark_url_info.to_redirect_url()
    assert (
        redirect_url
        == "http://0.0.0.0:4200/resources/0005/SQkTPdHdTzq_gqbwj6QR-A/-SSbnPK3Q7WWxzBT1UPpRg"
    )
    ...
    assert (
        redirect_url
        == "http://0.0.0.0:4200/resource/0001/cmfk1DMHRBiR4-_6HXpEFA/pLlW4ODASumZfZFbJdpw1g?version=20180604T085622Z"
    )
...
def test_ark_url_info_redirect_salsah_resource(settings):
    ...
    ark_url_info = ark_url_rust.ArkUrlInfo(
        settings, "ark:/00000/1/0803/751e0b8am.20190118T102919Z"
    )
    redirect_url = ark_url_info.to_redirect_url()
    ...
    ark_url_info = ark_url_rust.ArkUrlInfo(
        settings, "ark:/00000/080e-76bb2132d30d6-0.20190129"
    )
    redirect_url = ark_url_info.to_redirect_url()
    ...
    ark_url_info = ark_url_rust.ArkUrlInfo(
        settings, "ark:/00000/080e-76bb2132d30d6-0.2019111"
    )
    redirect_url = ark_url_info.to_redirect_url()
    assert redirect_url == "http://data.dasch.swiss/resources/2126045"
...
def test_ark_url_info_redirect_salsah_ark(settings):
    ...
    ark_url_info = ark_url_rust.ArkUrlInfo(
        settings, "ark:/00000/0002-779b9990a0c3f-6e.20190129"
    )
    redirect_url = ark_url_info.to_redirect_url()
    assert (
        redirect_url
        == "http://0.0.0.0:4200/resource/0002/Ef9heHjPWDS7dMR_gGax2Q?version=20190129"
    )
...
def test_conversion_to_resource_iri_with_ark_version_0(settings):
    ...
    ark_url_info = ark_url_rust.ArkUrlInfo(
        settings, "ark:/00000/0002-751e0b8a-6.2021519"
    )
...
def test_conversion_to_resource_iri_with_ark_version_1(settings):
    ...
    ark_url_info = ark_url_rust.ArkUrlInfo(
        settings, "ark:/00000/1/0002/0_sWRg5jT3S0PLxakX9ffg1.20210712T074927466631Z"
    )
    resource_iri = ark_url_info.to_resource_iri()
